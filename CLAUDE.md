...
- 코드 변경은 작업 단위로 커밋. 커밋 메시지는 한국어, 간결하게 (기존 히스토리 스타일).
- 보안: 사용자에게서 Supabase **`service_role` 키는 절대 받지 않는다.** `anon` 키만.

## 세션 시작 룰 (모든 Claude Code 세션 공통 — 반드시 준수)

새 채팅이 열리면 가장 먼저:

1. **메모리 인덱스 확인** — `~/.claude/projects/c--dev-plab01/memory/MEMORY.md` 의 항목 전체를 훑고, 다음 메모리는 반드시 본문까지 읽는다:
   - `current-status.md` — 마지막 진행 상태·다음 진입점
   - `work-style.md` — 사용자 검증된 답변·개발 스타일 (terse + 슬라이스 + 공통화 + 마스터-디테일 일관)
   - `work-mode-senior.md` — 시니어 모드: 묻기 전에 판단, sweep 적극
   - `skip-mid-task-confirm.md` — 합의된 sweep 안 세부 확인 금지
   - `auto-push-after-commit.md` — 커밋 = 즉시 push (Vercel 자동 배포)
   - `form-staged-changes.md` — 폼 변경은 [저장] 전까지 DB 비건드림

2. **사용자의 첫 메시지 의도가 기존 흐름과 맞물리면** `current-status.md` 의 "다음 진입점" 을 이어받아 진행한다. 새 작업이면 그 작업의 진행 상태를 새 메모리 또는 `current-status` 에 누적.

3. **응답 톤** — 한국어, 짧고 직접적. 헤더·표·코드블록 적극 사용. 진행 단계 설명은 1~2문장 + 결과 강조. 마무리 요약은 1~2문장 (변경 + 사용자 액션).

4. **작업 흐름** — 코드 변경 시:
   - 큰 슬라이스는 todo 로 분할
   - 단계마다 type check (`npx tsc --noEmit`)
   - 변경 단위로 `git add -A && git commit && git push` (한 호흡)
   - 커밋 메시지는 한국어, 1줄 제목 + 본문은 변경 요약
   - schema 변경 시 사용자에게 SQL 안내 (전체 재실행 또는 좁은 ALTER)

5. **마무리 시 메모리 갱신** — 진행 상태가 바뀌면 `current-status.md` 에 갱신. 새 결정·정책은 별도 메모리 파일 + `MEMORY.md` 인덱스에 한 줄 추가.

6. **UI 일관성** — 사용자가 한 번 합의한 디자인 패턴은 모든 페이지에 일괄 적용. 마스터-디테일·BackLink·subtext 한 줄·confirm 모달·필터 칩 등 패턴 일관.

7. **확인 vs 결정** — 데이터 모델 변경, schema 영향, 운영 로직 변경은 사용자 확인. UI 톤·라벨·CSS 등 디자인 영역은 senior 모드로 직접 결정 후 보고.

## 다른 Claude Code 세션 합류 시 (사용자 안내용)

새 채팅을 시작했는데 같은 흐름으로 이어가게 하려면 첫 메시지에:

> "MEMORY.md 와 current-status.md 보고 이전 흐름 그대로 이어서 진행해."

이것만 적으면 위 룰을 따라 동일한 패턴으로 동작합니다.
