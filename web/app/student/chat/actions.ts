...
export async function createStudentPost(formData: FormData) {
  ...
  revalidatePath("/student/chat/post");
  redirect("/student/chat/post");
}

// 학부모 게시글 답변 스레드 메시지 + 첨부 (sender='customer')
export async function sendStudentPostReply(formData: FormData) {
  ...
  revalidatePath(`/student/chat/post/${inquiryId}`);
}

// 학부모 1:1 채팅 메시지 + 첨부. redirect 제거 (revalidatePath 만 — same-path navigate 깜박임 회피)
export async function sendStudentChat(formData: FormData) {
  const inquiryId = String(formData.get("inquiry_id") ?? "");
  const body = String(formData.get("body") ?? "").trim();
  const files = formData.getAll("files") as File[];
  const realFiles = files.filter((f): f is File => f instanceof File && f.size > 0);
