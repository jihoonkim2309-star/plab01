...
type Report = { id: string; month: string; title: string; issuedAt: string };

const MOCK: Report[] = [
  { id: "r1", month: "2026.05", title: "5월 측정 리포트", issuedAt: "2026-06-01" },
];
...
async function fetchReports(): Promise<Report[]> {
  ...
  const { data } = await supabase
    .from("reports")
    .select("id, report_month, report_type, published_at")
    .eq("status", "발행완료")
    .eq("public_to_parent", true)
    .order("published_at", { ascending: false })
    .limit(50);
  type R = { id: string; report_month: string; report_type: string; published_at: string | null };
  return ((data ?? []) as R[]).map((r) => ({
    id: r.id,
    month: r.report_month.replace("-", "."),
    title: r.report_type,
  ...
  return (
    <>
      <div className="portal-topbar">
        ...
        <h1 style={{ flex: 1, textAlign: "center" }}>내 리포트</h1>
        <Bell size={20} />
      </div>
      <div className="portal-content">
        ...
        {reports.length === 0 ? (
          ...
          reports.map((r) => (
            <a key={r.id} href={`/student/reports/${r.id}`} className="card" style={{ display: "flex", alignItems: "center", gap: 12, textDecoration: "none", color: "#111" }}>
