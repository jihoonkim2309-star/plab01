...
export default function StaffSignup() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signUp({
      email, password,
      options: { data: { name, phone, applying_center_id: centerId || null, applying_role: role } },
    });
    if (error) {
    ...
    setMsg("가입 신청이 접수되었습니다.\n슈퍼어드민 승인 후 로그인 가능합니다.");
    setLoading(false);
  }
  ...
  return (
    <div style={{ flex: 1, padding: "24px", overflowY: "auto", background: "#fff" }}>
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        ...
        <h1 style={{ fontSize: 18, fontWeight: 800 }}>관리자 가입</h1>
        <p style={{ fontSize: 12, color: "#6f7d78", marginTop: 4 }}>슈퍼어드민 승인 후 활성</p>
      </div>
      ...
      <form onSubmit={onSubmit}>
        ...
        {msg && (
          <div style={{
            ...
            borderRadius: 8, fontSize: 12, marginBottom: 12, whiteSpace: "pre-wrap", lineHeight: 1.5,
          }}>{msg}</div>
        )}
        ...
        <div style={{ textAlign: "center", marginTop: 14, fontSize: 12 }}>
          이미 계정이 있나요?{" "}
          <a href="/staff/login" style={{ color: "#92400e", fontWeight: 700, textDecoration: "none" }}>
            로그인
          </a>
        </div>
      </form>
